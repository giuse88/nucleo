...
/* PRIMITIVA ************************************************************ 
 * Primitiva accessibile a livello utente che riporta l'ora del BIOS.   *    
 * Il formato dell'ora sara il seguente : 				*
...
extern "C" natl  c_get_time () { 
  ...
  // verifico se il formato dell'orologio e binario o bcd
...
/*PRIMITIVA ********************************************************
 * primitiva accessibile da livello utente che prelava la data	   *
 * corrente dal BIOS. Riporta la data nel seguente modo :	   *
 * primao byte  :  giorno 1-31 					   *
...
extern "C" void c_get_error ( natl * e , char *buf, natl size_buffer) {  

  
  if (!buf || !errno || !size_buffer) 
    return; 
  
  strncpy(buf,str_errno, size_buffer); 
  *e=errno; 
...
// Funzione che esegue l'inizializzazione del file system 
