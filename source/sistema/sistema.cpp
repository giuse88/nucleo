...
void * io_global_info=NULL; 	// puntatore zona dati globale 
natl io_global_size=0; 		// grandezza zona dati 
...
extern des_sem array_dess[2][MAX_SEM];  /* separiamo i semafori sistema/IO da quelli utente */
...
bool sem_valido(natl sem, natl liv);
// )


// [4.11]
extern "C" void c_sem_wait(natl sem, natl liv)
{
	des_sem *s;

// (* una primitiva non deve mai fidarsi dei parametri
	if (!sem_valido(sem, liv)) {
		flog(LOG_WARN, "semaforo errato: %d", sem);
		abort_p();
	}
// *)

	s = &array_dess[liv][sem];
	(s->counter)--;
...
// [4.11]
extern "C" void c_sem_signal(natl sem, natl liv)
{
	des_sem *s;
	proc_elem *lavoro;
...
void *alloca(natl dim, natl align = 3);	// [4.14]
...
struct des_pf {			// [6.3]
	...
	union {
		struct { 
			natl	residente;	// pagina residente (> 0) o meno (= 0)
...
const natl ntab_video_condiviso	  = NTAB_VID_C;
...
const natl i_video_condiviso   = i_io_condiviso      + ntab_io_condiviso;
const natl i_pci_condiviso     = i_video_condiviso   + ntab_video_condiviso;
...
const natl dim_video_condiviso	 = ntab_video_condiviso	  * DIM_MACROPAGINA;
...
natb* const inizio_video_condiviso   = fine_io_condiviso;
natb* const fine_video_condiviso     = inizio_video_condiviso   + dim_video_condiviso;
natb* const inizio_pci_condiviso     = fine_video_condiviso;
	...
	)
{
	...
	if (risu == 0) {
		...
		flog(LOG_WARN, "page fault non risolubile: eip=%x, ind=%x", eip, ind_virt_non_tradotto);
		abort_p();
		// )
	}
}
...
void carica(natl indice) // [6.4][10.5]
{
	...
	case DIRETTORIO:
		{ addr pdir = readCR3();
		  ...
		  copy_des(pdir, dest, i_video_condiviso,   ntab_video_condiviso);
	...
	// *)
	default:
		flog(LOG_ERR, "carica(%d): contenuto=%d", indice, ppf->contenuto);
		abort_p();
	}
}
...
bool in_spazio_condiviso(addr v, natl n)
{
	natb *vs = static_cast<natb*>(v),
	     *ve = vs + n * DIM_PAGINA;

	return (vs >= inizio_io_condiviso && ve <= fine_io_condiviso) ||
	       (vs >= inizio_utente_condiviso && ve <= fine_utente_condiviso);
}

extern "C" bool c_resident(addr ind_virt, natl quanti, bool residente) // [10.6]
{
	...
	if ( !in_spazio_condiviso(ind_virt_pag, np) ) {
		flog(LOG_WARN, "resident(%x, %d, %s): non in spazio condiviso", ind_virt, quanti, residente? "true" : "false");
		abort_p();
	}
	for (natl i = 0; i < np; i++) {
		sem_wait(pf_mutex);
...
bool pci_find_class(natw& w, natb code[])
{
	for ( ; w != 0xFFFF; w++) {
		...
		for (i = 0; i < 3; i++) {
			...
			//flog(LOG_DEBUG, "code[%d] = %2x; work[%d] = %2x", i, code[i], i, work[i]);
			if (code[i] != 0xFF && code[i] != work[i])
				break;
		}
		//flog(LOG_DEBUG, "w = 0x%4x", w);
		if (i == 3) {
			for (i = 0; i < 3; i++)
				code[i] = work[i];
				//flog(LOG_DEBUG, "Ho individuato il dispositivo");
			return true;
		}
	}
	//flog(LOG_DEBUG, "Impossibile individuare il dispositivo");
	return false;
}
...
extern "C" natl c_pci_find(natl code, natw i)
{
	...
	//flog(LOG_DEBUG, "pcode = &code = %x, (*pcode) = %x, code = %x", pcode, (*pcode), code);
	
	//natb* pcode = (natb*)code;
	//flog(LOG_DEBUG, "pcode = code = %x, (*pcode) = %2x, &code = %x", pcode, (*pcode), &code);
	natw w, j = 0;
	for(w = 0; w != 0xFFFF; w++) {
...
enum hd_cmd {			// [9.3]
	...
	IDENTIFY	= 0xEC, 
	// *)
	// (* comandi per il DMA/bus mastering
	WRITE_DMA	= 0xCA,
	READ_DMA	= 0xC8,
	// *)
	WRITE_SECT	= 0x30,
...
{	
	...
	/* trasformiamo l'if in uno switch per gestire i vari casi */
...
bool crea_finestra_VID(addr direttorio, addr framebuffer);
natl crea_dummy();
bool init_pe();
...
extern "C" void cmain (natl magic, multiboot_info_t* mbi)
{
	...
	if (mbi->flags & MULTIBOOT_INFO_FRAMEBUFFER_INFO) {
		if (mbi->framebuffer_width  != VID_W ||
		    mbi->framebuffer_height != VID_H ||
		    mbi->framebuffer_bpp    != VID_B)
		{
			flog(LOG_ERR, "risoluzione errata: %dx%dx%d",
					mbi->framebuffer_width,
					mbi->framebuffer_height,
					mbi->framebuffer_bpp);
			goto error;
		} else {
			flog(LOG_DEBUG, "fb: %x", static_cast<natl>(mbi->framebuffer_addr));
			framebuffer = reinterpret_cast<addr*>(mbi->framebuffer_addr);
			graphic_mode = 1;
		}
	} 
	// *)
	...
	if (mbi->flags & MULTIBOOT_INFO_CMDLINE) {
		for (arg = str_token( (char*) mbi->cmdline, &cont);
		     cont != 0 || arg != 0;
	...
	// (* memoria PCI in memoria virtuale
	if (!crea_finestra_PCI(direttorio))
		goto error;
	flog(LOG_INFO, "Mappata memoria PCI in memoria virtuale");
	// *)
	
	// (* memoria video in memoria virtuale
	if (!crea_finestra_VID(direttorio, framebuffer))
		goto error;
	flog(LOG_INFO, "Mappata memoria video in memoria virtuale");
...
static void htostr(str vbuf, natl l, int cifre = 8)
{
...
// abbiamo allocati 
natl sem_allocati[2] = { 0, 0 };
natl alloca_sem(natl liv)
{
	natl i;

	if (sem_allocati[liv] >= MAX_SEM)
		return 0xFFFFFFFF;
...
bool sem_valido(natl sem, natl liv)
{
	return sem < sem_allocati[liv];
}

// parte "C++" della primitiva sem_ini [4.11]
extern "C" natl c_sem_ini(int val, natl liv)
{
	natl i = alloca_sem(liv);

	if (i != 0xFFFFFFFF)
		array_dess[liv][i].counter = val;
...
void sem_dump()
{
	...
	char buf[BUF_SIZE + 1];
	for (natl liv = 0; liv < 2; liv++) {
		for (natl i = 0; i < sem_allocati[liv]; i++) {
			des_sem *s = &array_dess[liv][i];
			proc_elem *p = s->pointer;
			if (p) {
				int l = 0;
				l = snprintf(buf + l, BUF_SIZE - l, "%c%d:", (liv? 'U': 'S'), i) - 1;
				while (p) {
					l += snprintf(buf + l, BUF_SIZE -l, "%d ", p->id) - 1;
					p = p->puntatore;
				}
				flog(LOG_DEBUG, buf);
			}
...
// calcola quanti byte mancano da 'p' al prossimo
// indirizzo allineato con 'align'
natl padding(addr p, natl align)
{
...
void* alloca(natl dim, natl balign)
{
...
extern "C" void* c_phys_alloc(natl dim, natl balign)
{
	return alloca(dim, balign);
}

extern "C" void c_phys_free(void* addr)
{
	dealloca(addr);
}
...
proc_elem* crea_processo(void f(int), int a, int prio, char liv, bool IF)
{
	...
	des_proc	*pdes_proc,		// descrittore di processo
			*father_proc;		// descrittore del processo padre
	...
	dpf[indice_dpf(pdirettorio)].pt.residente = 1;
...
void distruggi_processo(proc_elem* p)
{
	...
	dealloca(pdes_proc->iostate); // elimino l'area privata di un processo 
	rilascia_tss(p->id >> 3);
	dealloca(pdes_proc);
}
...
{
	des_proc *p;
	for (natl i = MIN_ID; i <= MAX_ID; i++) {
		natl id = i << 3U;
		if ( (p = des_p(id)) ) {
			natl esp = p->contesto[I_ESP];
			natl dp = get_despag(id, (addr)esp);
			natl eip = *((natl*)((natl)extr_IND_F(dp) | (esp & 0x00000FFF)));
			flog(LOG_DEBUG, "%d: %c %x",
				id, 
				(p->cpl == LIV_SISTEMA? 'S' : 'U'),
				eip);
		}
	}
}
...
// mappa le ntab pagine virtuali a partire dall'indirizzo virt_start agli indirizzi fisici
...
bool crea_finestra_VID(addr direttorio, addr framebuffer) {
	...
	return sequential_map(direttorio, 
			framebuffer,
			inizio_video_condiviso,
			ceild(VID_W * VID_H * (VID_B / 8 ), DIM_PAGINA),
			BIT_PWT | BIT_RW);
}
...
bool aggiungi_pe(proc_elem *p, natb irq)
{
	...
	des_p(p->id)->disp[0] = irq;
	if (a_p[irq] == a_p_save[irq]) {
		distruggi_processo(a_p_save[irq]);
		dealloca(a_p_save[irq]);
		a_p[irq] = a_p_save[irq] = 0;
	}
	inserimento_lista(a_p[irq], p);
	ioapic_set_MIRQ(irq, false);
	return true;
...
extern "C" void inviaEOI();
extern "C" void c_wfi()
{
	des_proc *p = des_p(esecuzione->id);
	natl irq = p->disp[0];

	inserimento_lista(a_pp[irq], esecuzione);
	if (a_p[irq]) {
		proc_elem *work;
		rimozione_lista(a_p[irq], work);
...
{
	natb errore;
	des_part* p;     //  partizione fisica mbr
	partizione *estesa=NULL, **ptail=NULL;  // partizioni logiche lista
	des_ata* p_des = &hd[ata];     // canali 
	static natb buf[512];
	natl settore;
	partizione* pp=NULL;
	//	partizione* tmp =NULL; 
...
bool hd_init()
{
	...
	for (natl i = 0; i < A; i++) {			// primario/secondario
		...
		if ( (p_des->mutex = c_sem_ini(1, 0)) == 0xFFFFFFFF ||
		     (p_des->sincr = c_sem_ini(0, 0)) == 0xFFFFFFFF)
		{
...
/********************************************************************************
 * PRIMITIVA SISTEMA								*
 * E' una primitiva che analizza la lista delle partizioni.Cerca una 		*
 * partizione di un determinato tipo a partire da un indice su un Hard disk.	*
 * ATA    : canale del disco 							*
 * DRV    : driver del disco 							*
 * TIPO   : tipo partizione 							*
 * INDICE : indice da cui iniziare la ricerca					*
 * Ritorna l'indice della partizione all'interno della lista se ha successo 	*
 * altrimenti -1								*	
 ********************************************************************************/


extern "C" int  c_get_partizione ( natl ata, natl drv, natl tipo, int indice ) { 
  
  partizione * list_part=NULL;  
  int  i=0; 
  ...
  //prelevo la lista delle partizioni
  list_part=(hd[ata].disco[drv].part);
  ...
  while (list_part)
    if ( tipo == (natl )list_part->type && indice <= i) 
      return i; 
    else  {
      i++; 
...
/**********************************************************
 * PRIMITIVA LIVELLO SISTEMA 				  *
 * Questa primitiva esegue la lettura su una determinata  *
 * partizione identificata dai valori passati come para-  *
 * metri formali e l'inserisce all'indirizzo buf.	  *
 * ATA            : canale ATA				  *
 * DISCO          : driver del disco 			  *
 * part           : indice partizione 			  *
 * indice_settore : primo settore da leggere     	  *
 * n_blocchi      : numero blocchi da leggere 		  *
 * Se ha avuto successo ritorna zero altrimenti -1 se	  * 
 * si è verificato un errore .				  *
 **********************************************************/

extern "C" int c_read_part_n ( natl ata , natl disco,  natl indice_partizione, natl indice_settore, natl n_blocchi, void  * buf)
 { 

  partizione * part=NULL;
  natl settore=0;
  natb errore=0;  
  drive * tmp_disco=NULL; 

  if ( ata < 0 || ata >=2 )  {
	flog(LOG_WARN, " ERRORE: Ata errato  %d", ata);
	return -1; 
  }

  if ( disco < 0 || disco >=2 )  {
	flog(LOG_WARN, "ERRORE: Disco errato %d", disco);
	return -1; 
  }
	
  if ( indice_partizione <= 0 ) { 
    flog(LOG_WARN,"ERRORE: Indice_partizione errato %d", indice_partizione); 
    return -1; 
  } 

   if ( n_blocchi <=0) { 
       flog(LOG_WARN, "ERRORE : Numero blocchi errato %d", n_blocchi);
	   return -1; 
   }

  // PRELEVO PARTIZIONE 
  part= hd_find_partition(ata, disco,indice_partizione); 

//  des_ata* pdes_ata = &hd[ata];
//   partizione*tmp=pdes_ata->disco[disco].part; 
/*  
   while (tmp) { 
     flog(LOG_DEBUG,"INFO %d TYPE %x", tmp->first,tmp->type);
     tmp=tmp->next;
   }
  */
   if (part == NULL) { 
    flog(LOG_WARN, "ERRORE :Partizione non trovata ata :%d disco :%d indice %d",ata,disco, indice_partizione ); 
    return -1; 
  }

  
 
  if ( indice_settore <  part->first && (( indice_partizione + n_blocchi) >  ( part->first + part->dim))){ 
       flog(LOG_WARN, "ERRORE: lettura patizione fuori settore"); 
       return -1; 
   }

	
	tmp_disco=&(hd[ata].disco[disco]);    // prelevo descrittore disco 
        settore=indice_settore+ part->first;  // offest sulla partizione 
  
    if (tmp_disco->dma)
       dmareadhd_n(ata, disco, static_cast<natw*>(buf), settore, n_blocchi, errore);
   else 
        readhd_n(ata, disco, static_cast<natw*>(buf),settore,n_blocchi, errore);

     if (errore != 0) { 
       flog(LOG_ERR, "Impossibile leggere il blocco %d Partizione %d", n_blocchi, indice_partizione);
		hd_print_error(ata, disco, n_blocchi, errore);
		return -1;
     }
...
/**********************************************************
 * PRIMITIVA LIVELLO SISTEMA 				  *
 * Questa primitiva esegue la scrittura di buf su una 	  *
 * determinata partizione identificata dai valori passati *
 * come parametri formali.				  *
 * ATA            : canale ATA				  *
 * DISCO          : driver del disco 			  *
 * part           : indice partizione 			  *
 * indice_settore : primo settore della scrittura 	  *
 * n_blocchi      : numero blocchi da scriver 		  *
 * Se ha avuto successo ritorna zero altrimenti -1 se	  * 
 * si è verificato un errore .				  *
 **********************************************************/

extern "C" int c_write_part_n ( natl ata , natl disco,  natl indice_partizione, natl indice_settore, natl n_blocchi, void  * buf) 
...
{ 
  ...
  if ( indice_settore <  part->first && (( indice_partizione + n_blocchi) >  ( part->first + part->dim))){ 
	...
	tmp_disco=&(hd[ata].disco[disco]); 
    settore=indice_settore+ part->first;  // offest sulla partizione 

	
	// chiamata al driver che esegue il trasferimento 	
    if (tmp_disco->dma) 
       dmawritehd_n(ata, disco, static_cast<natw*>(buf), settore, n_blocchi, errore);
   else 
        writehd_n(ata, disco, static_cast<natw*>(buf),settore,n_blocchi, errore);
      
     


     if (errore != 0) { 
       flog(LOG_ERR, "Impossibile scrivere il blocco %d Partizione %d", n_blocchi, indice_partizione);
		hd_print_error(ata, disco, n_blocchi, errore);
		return -1;
     }
...
// )
// ( [P_LOG
// Il log e' un buffer circolare di messaggi, dove ogni messaggio e' compsto 
// da tre campi:
...
bool log_init_usr()
{
	if ( (log_buf.mutex = c_sem_ini(1, 0)) == 0xFFFFFFFF)
		goto error;

	if ( (log_buf.sync = c_sem_ini(0, 0)) == 0xFFFFFFFF)
		goto error;
...
void do_log(log_sev sev, const natb* buf, natl quanti)
{
	...
	if (log_buf.sync && log_buf.nmsg > 0) {
		proc_elem* lavoro;
		des_sem *s = &array_dess[0][log_buf.sync];
		if (s->counter < 0) {
			s->counter++;
			rimozione_lista(s->pointer, lavoro);
			inserimento_lista(pronti, lavoro);
...
void flog(log_sev sev, cstr fmt, ...)
{
	...
	if (l > 1)
		...
		flog(LOG_DEBUG, "flog(%d, \"%s\")=%d", sev, fmt, l);
}
...
// lettura dallo swap (da utilizzare nella fae di inizializzazione)
		...
		if (extr_P(dt)) {	  
			...
			for (int k = 0; k < 1024; k++) {
				...
				if (extr_P(dp)) {
					...
					dpf[indice_dpf(ind_fis)].pt.residente = 1;
				}
			}
		}
	}
...
extern "C" void c_dump(natl tipo)
{
	switch (tipo) {
	case DUMP_IOAPIC:
		ioapic_dump_rt(0, 23);
	...
	case DUMP_SEM:
		...
		break;
	default:
		flog(LOG_WARN, "tipo di dump sconosciuto: %d", tipo);
		break;
	}
}
// )
...
extern "C" addr c_graphic_mode() {
	if (graphic_mode)
		return reinterpret_cast<addr*>(inizio_video_condiviso);
	else
		return 0;
}
...
// ( [P_INFO]
// Primitive per la gestione dell'area privata di un processo 


#define EINVAL 		22       // errore parametri incoretti
#define ENOMEM		12	 // errore memoria 


extern "C" natl c_get_id() {
	return esecuzione->id;
}

extern "C" natl c_get_id_p() {
	des_proc *p_des = des_p(esecuzione->id);
	return p_des->p_id;
}

/* PRIMITIVA **************************************************************
 * La primitiva chiede al nucleo di allocare una zona di memoria globale  *
 * di 'size' byte e di inizializzarla con il contenuto dell'array 'init'. *
 *  INIT : puntatore al vettore con il quale si vuole inizializzare 	  *
 * 	   l'area globlae. 						  *
 *  SIZE : grandezza area globale 					  *
 * La primitiva fallisce se una zona del genere � gi� stata creata. Se ha *
 * successo riporta zero.						  *
 **************************************************************************/

extern "C"  natl c_io_space_init(natb init[], natl size) { 
 
  if ( !(init && size))  // verifico la consistenza dei parametri 
      return EINVAL; 
  
  if (io_global_info)	// verifico che non sia stata gi� allocata
      return ENOMEM; 
  
  // alloco lo spazio neccessario 
  io_global_info=alloca(size, 1); 
  
  if (!io_global_info) 
      return ENOMEM;
  
  memset(io_global_info,0, size); 
  memcpy(io_global_info, init, size); 	// copio i dati di inizializzazione
...
/* PRIMITIVA ****************************************************************************
 * Primitiva che mi permette di leggere dei valori nell'area primavata di un processo   *
 * OFFSET : offset all'interno dell'area privata sul quale eseguire la scrittura        *
 * VETTI  : puntatore in cui inserire i dati letti 					*
 * QUANTI : numero di byte da leggere							*
 * Ritorna zero se ha successo, altrimenti un codice di errore positivo 		*
 * **************************************************************************************/


extern "C" natl c_io_space_read(natl offset, natb * vetti, natl quanti) {
  

  	des_proc *p_des = des_p(esecuzione->id);	
	
	if ( !vetti || !quanti ) { 
	    flog(LOG_WARN, "Errore parametri off:%x addr:%x n:%x ",offset , vetti , quanti );
	    return EINVAL; 
	}  
	  
	
	if (!p_des) {
		flog(LOG_WARN, "Processo non esistente");
		abort_p();
	}
  
	if (!p_des->iostate || !p_des->iostate_size ) { 
		flog(LOG_WARN, "Memoria non presente : PID %d", esecuzione->id);
		return ENOMEM; 
	}
  
	if (offset+quanti > p_des->iostate_size) { 
		flog(LOG_WARN, "Indirizzo fuori memoria");
		return ENOMEM;
	}
  
	//copiamo i dati nell'indirizzo vetti
	memset(vetti,0, quanti); 
	memcpy(vetti,static_cast<natb*>(p_des->iostate)+offset, quanti); 
	
	return 0; 
}
  
/* PRIMITIVA ****************************************************************************
 * Primitiva che mi permette di scrivere dei valori nell'area primavata di un processo  *
 * OFFSET : offset all'interno dell'area privata sul quale eseguire la scrittura        *
 * VETTI  : puntatore ai dati da scrivere 						*
 * QUANTI : numero di byte da scrivere 							*
 * Ritorna zero se ha successo, altrimenti un codice di errore positivo 		*
 * **************************************************************************************/

extern "C" natl c_io_space_write(natl offset, const natb vetto[], natl quanti) {

	des_proc *p_des = des_p(esecuzione->id);
	
	if ( !vetto || !quanti ) { 
	    flog(LOG_WARN, "Errore parametri");
	    return EINVAL; 
	}  
	...
	//copiamo i dati nell'indirizzo vetti
	memcpy(static_cast<natb*>(p_des->iostate)+offset,vetto, quanti); 
