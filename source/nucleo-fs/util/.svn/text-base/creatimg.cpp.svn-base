...
const uint fine_io_condiviso     = (NTAB_SIS_C + NTAB_SIS_P + NTAB_MIO_C)              		    * DIM_MACROPAGINA;
const uint fine_utente_condiviso = (NTAB_SIS_C + NTAB_SIS_P + NTAB_MIO_C + NTAB_VID_C + NTAB_USR_C) * DIM_MACROPAGINA;
...
void crea_heap(uint start, uint end, bool user)
{
	...
	for (int i = indice_direttorio(start) + 1; i < indice_direttorio(end); i++)
	{
		descrittore_tabella* pdes_tab = &main_dir.entrate[i];
		pdes_tab->a.block = 0;
		pdes_tab->a.PWT   = 0;
		pdes_tab->a.PCD   = 0;
		pdes_tab->a.US	  = (user? 1 : 0);
		pdes_tab->a.RW	  = 1;
		pdes_tab->a.P     = 1;
	}
}
...
int main(int argc, char* argv[])
{
	...
	crea_heap(last_address, fine_io_condiviso, false);


	do_map(argv[3], 1, superblock.user_entry, last_address);
	superblock.user_end = addr(last_address);
	crea_heap(last_address, fine_utente_condiviso, true);
